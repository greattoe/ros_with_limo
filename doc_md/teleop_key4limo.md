...
앞서 Limo 로봇에 연결한 키보드, 마우스와 Limo 로봇 후방의 모니터를 이용하여 준비한 휴대폰 핫스팟에 연결 후 터미널을 열어  아래 명령을 실행하여 `openssh-server` 설치한다. 
...
`ROS_MASTER_URI`는 `roscore`가 실행될 시스템의 프로토콜 밑, 포트번호를 포함한 IP주소이다. 이 후 실습에서 `roscore`는 노트북 PC에서 구동할 것이므로 이 경우 `http://10.42.0.2:11311`이 되겠다.
...
#### 3. `limo_pkg`를 만들고 Limo로봇을 키보드로 원격제어하는 `remote_limo.py`노드를 만들어보자. 
